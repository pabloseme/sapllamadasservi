<?php


$nombre=trim($_POST['usuario']);
$clave=trim($_POST['clave']);
// Host name
$host = "https://190.2.251.208";

// Port
$port = 50000;

//sesion
$sesion="";

// Login credentials
$params = [
    "UserName" => $nombre,
    "Password" => $clave,
    "CompanyDB" => "YUHMAKSA_PRD",
];

$curl = curl_init();
curl_setopt($curl, CURLOPT_URL, $host . ":" . $port . "/b1s/v1/Login");
curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, FALSE);
curl_setopt($curl, CURLOPT_VERBOSE, 1);
curl_setopt($curl, CURLOPT_POST, true);
curl_setopt($curl, CURLOPT_HTTPHEADER,(array('Content-Type:application/json')));
curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($params));

$response = curl_exec($curl);

//echo curl_error($curl);//

//var_dump($response);

//echo $response ;
//echo json_encode($params);
//echo json_encode($response);
//echo $response;//

$obj = json_decode($response);
$varsesion=$obj->{'SessionId'};

///iniciar sesion con el identificador que me devuelve el logeo
session_id($varsesion);
session_start();
ob_start();


///echo "Numero de Sesion :".$varsesion."<br />";

$routeId = "";

curl_setopt($curl, CURLOPT_HEADERFUNCTION, function($curl, $string) use (&$routeId){
    $len = strlen($string);

    if(substr($string, 0, 10) == "Set-Cookie"){
        preg_match("/ROUTEID=(.+);/", $string, $match);

        if(count($match) == 2){ 
            $routeId = $match[1];
        }
    } 
    return $len;
});

curl_exec($curl);


$headers[] = "Cookie: B1SESSION=" . $varsesion . "; ROUTEID=" . $routeId . ";";

$_SESSION['logged_in_user_id'] = "Cookie: B1SESSION=" . $varsesion . "; ROUTEID=" . $routeId . ";";


echo $headers[0];
echo "<br />";
echo "<br />";

echo $_SESSION['logged_in_user_id'];
//echo "ROUTERID :".$routeId;

echo "<br />";
echo "<br />";


echo "<br />";
echo "<br />";


//header("location:index.html");  //direcciona a otra pagina
header("location:llamadas.html");  //direcciona a otra pagina

?>


